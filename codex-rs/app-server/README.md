...
- Start (or resume) a thread: Call `thread/start` to open a fresh conversation. The response returns the thread object and you’ll also get a `thread/started` notification. If you’re continuing an existing conversation, call `thread/resume` with its ID instead. If you want to branch from an existing conversation, call `thread/fork` to create a new thread id with a logical snapshot of the source history. Local persistence may store that snapshot as a bounded reference plus a child-local suffix instead of duplicating the source JSONL bytes; reads and model context are equivalent, but the source cannot be deleted while a persisted child still references it. Like `thread/start`, `thread/fork` also accepts `ephemeral: true` for an in-memory temporary thread.
...
- `thread/fork` — fork an existing thread into a new thread id with a logical snapshot of the stored history; the local store may persist the inherited prefix as a bounded reference and write only the child’s delta. This avoids duplicating large JSONL prefixes, but means a persisted source cannot be deleted while any child still references it; delete the child (or all descendants) first. The reference format requires a binary that understands `history_base`; do not downgrade a CODEX_HOME containing such children to an older binary. Pass an optional `lastTurnId` to snapshot history only through that turn, inclusive, and drop later turns from the fork. An in-progress `lastTurnId` boundary is rejected. Experimental `beforeTurnId` instead snapshots history strictly before the referenced turn, including when that turn is in progress, and cannot be combined with `lastTurnId`. If both boundaries are null while the source thread is mid-turn, the fork records the same interruption marker as `turn/interrupt` instead of inheriting an unmarked partial turn suffix. The returned `thread.forkedFromId` points at the source thread when known. Accepts `ephemeral: true` for an in-memory temporary fork, emits `thread/started` (including the current `thread.status`), and auto-subscribes you to turn/item events for the new thread. Experimental clients can pass `excludeTurns: true` when they plan to page fork history via `thread/turns/list` instead of receiving the full turn array immediately, or `deferGoalContinuation: true` to carry the source thread's current goal into the fork and run an explicit turn before automatic continuation resumes. Deferred goal continuation is persisted until that turn starts and cannot be combined with `ephemeral: true`. Accepts the same permission override rules as `thread/start`.
...
For persisted local forks, the snapshot can be reference-backed: the child stores a bounded `history_base` position and only its own suffix, while replay resolves the inherited prefix from the source rollout. This is a logical snapshot rather than a physical copy, so deleting the source is rejected until every referencing child/descendant is removed. The reference format is supported only by binaries that understand `history_base`; older binaries sharing the same CODEX_HOME are not a supported downgrade target.
