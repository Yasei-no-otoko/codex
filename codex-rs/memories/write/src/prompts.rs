...
pub fn build_stage_one_input_message(
    ...
    // The fixed framing can grow independently of the rollout (for example,
    // with an unusually long path). Keep the invariant fail-closed even in
    // that case rather than emitting an oversized model-visible item.
    Ok(truncate_text(
        &rendered,
